...
def test_interrupted_install_preserves_previous_active_runtime(bundle: Path, tmp_path: Path) -> None:
    ...
    assert _run([str(_venv_python(runtime / "current" / "venv")), "-m", "zero_mem.cli", "--version"], env=env, cwd=tmp_path).stdout.strip() == "zero-mem 1.3.4"
