...
For every supported callback, the bridge must enforce this exact order:

1. Copy the incoming hook payload.
2. Perform security-safe structural normalization only.
3. Redact or reject sensitive content.
...
7. Construct and validate the Increment 1 envelope.
8. Append through the Increment 3 capture store.

Security-safe structural normalization may copy and normalize approved containers, but it must never stringify unsupported objects, call `repr()` on unknown values, log raw values, hash raw content, or persist temporary raw representations.

For each supported callback:

- Return the original hook-compatible result unchanged.
- Do not invoke storage, hashing, or envelope construction before redaction succeeds.
- Missing optional identity/correlation fields remain null or empty according to the Increment 1 contract.

## Hook payload to normalized envelope mapping

The mapping stage occurs only after the redaction step above:

1. Map the sanitized structure into the normalized event type/source.
2. Resolve explicit bridge/session/profile/project metadata.
3. Compute the sanitized content hash.
4. Construct and validate the Increment 1 envelope.
5. Append only through Increment 3 `CaptureStore.append`.

Original hook arguments and Hermes control results remain untouched.

## Capture-rate accounting

The denominator is the set of expected supported logical events generated by the controlled harness.

Each expected logical event contributes at most one successful result. A logical event is successfully accounted for when either:

- it is appended as one valid sanitized record; or
- it returns an accepted duplicate result and the corresponding original record is confirmed to exist.

Use this formula:

```text
capture_rate = successfully_accounted_logical_events / expected_supported_logical_events * 100
```

Do not count both an append and later duplicate attempts for the same logical event. Deferred and unsupported event classes remain outside the denominator.
...
- account for each expected supported logical event at most once;
- count an event as successful when appended or when an accepted duplicate result confirms the original record exists;
- exclude deferred event classes from the denominator;
- calculate `successfully_accounted_logical_events / expected_supported_logical_events * 100`;
...
| Capture rate | Controlled supported logical-event harness uses `successfully_accounted_logical_events / expected_supported_logical_events * 100`, counts each logical event once, accepts append or confirmed duplicate, and reaches `>=99%`; deferred classes excluded from denominator |
