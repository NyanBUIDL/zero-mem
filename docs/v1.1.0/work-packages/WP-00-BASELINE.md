...
One reviewable planning layer identifies the released baseline, exact reconciliation point, delta types, authority order, reconciled finding states/owners/closure tests, complete WP set, acyclic dependency graph, source freeze, and Git publication gate.
...
WP-01 through WP-19 design approval and every future implementation authorization.
...
- Verify WP-00–WP-19 presence and required sections.
...
- WP-00–WP-19 contain every mandatory field; no implementation WP is active or authorized.
- Dependency graph is acyclic, local links resolve, exact-master code links resolve, and no duplicate closure owner exists.
