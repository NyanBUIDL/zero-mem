...
| WP V130-05 (WIP) | Generator deterministic: 2 lần chạy byte-identical (sha256 `81127ab9…`, D1 ✓). Benchmark synthetic N=5,000 kết quả (`zero-mem-dev-data/evidence/v130-wp05-synthetic-results.json`): OR-fallback recall ✓ (mode=or_fallback, 50 hits), precision guard ✓ (AND mode giữ nguyên), ks leak=0 ✓ (50/50 hits, giao=0), state promotion ✓ (5 states vào primary, budget ≤5/3/8, t_build 24.6ms), temporal as-of before-supersession valid=true + after facts=1 ✓. temporal_index_rows=4866, m4_projected=832, t_ingest≈4.2s. Fixtures ngoài git track ✓. Tracker SQLite≥3.35 mục 2: benchmark runner chạy trên cùng môi trường dev-data; check version khi chạy máy khác — TODO ghi vào closure. Còn lại: real-corpus pipeline redaction gate, token-savings đo, Gate D e2e review, closure handoff. | IN_PROGRESS |
| (chưa có) | | |
