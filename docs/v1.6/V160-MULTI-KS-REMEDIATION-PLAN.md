...
- **Review follow-up (P1 — junction là NGUỒN DUY NHẤT, fail-closed):** bỏ OR singular fallback (`NOT EXISTS junction AND zm_meta.knowledge_space_id IN ...`) khỏi `_ks_predicate` → chỉ correlated EXISTS; defensive re-check query_events/get_event/get_trace chỉ dùng junction map (bỏ `or _row_ks_ids`); giữ API alias `row_knowledge_space_id` cho unit callers cũ (DEF-010/011/012 direct calls) nhưng production event-path KHÔNG dùng alias làm fallback. RED 1 failed (singular-no-junction bị leak) → GREEN; legacy đúng: fixture DEF-010 đổi sang **v12 → insert singular → migration runner → v13 (junction backfill)** — grant authorize qua junction; test `test_legacy_v12_upgrade_authorizes_via_junction` guard. Adjacent **511 passed**; full suite **3583 passed / 6 failed** (pre-classified env/Windows — KHÔNG gọi là green) / 38 skipped / 11 errors (installer quirk, pre-classified). DEF-034 stays OPEN.
- **Review follow-up P2 (evidence + test chính xác):** đổi tên `test_singular_no_junction_fail_closed` → `test_singular_no_junction_fail_closed_base_scope` (mô tả chính xác — base scope, không có explicit grant); thêm `test_explicit_grant_missing_junction_fail_closed` — event **cross-profile** (`zm_meta.knowledge_space_id='A'`, junction row bị xóa) + **explicit knowledge-space grant A** → `query_events` + `get_event` đều fail-closed. **RED trên baseline 8aab66d** (singular fallback leak: `evX` được trả) → **GREEN trên HEAD** (focused 30 passed, adjacent 512 passed). Không sửa production code (implementation đã đúng).
- **Evidence portable (in-repo, sau clone):** raw logs commit tại `audit/evidence-v160-c2/` (trong repo): `c4-red-6failed.txt`, `c4-green-11passed.txt`, `c4-adjacent-475passed.txt`, `c4-fullsuite-6failed.txt`, `c4fu-red-1failed.txt`, `c4fu-green-47passed.txt`, `c4fu-adjacent-511passed.txt`, `c4fu-fullsuite-6failed.txt`, `c4p21-red-baseline-8aab66d.txt` + `SHA256SUMS.txt` (hash toàn bộ raw logs).
- **Reproducibility (reconcile 2026-08-26, HEAD b8f818a0fcb1d01af47c754bf0530be2159f98a5):**
  - Python: `.venv\Scripts\python.exe` = 3.13.15 (standalone build); cwd = repo root; env: TEMP/TMP/TMPDIR = `zero-mem\.tmptest`, HOME/USERPROFILE = `zero-mem\.tmptest\zm-home`, XDG_*_HOME dưới zm-home; `-p no:cacheprovider`.
  - **Focused (4-file required matrix):** `python -m pytest -q tests/unit/test_v160_c4_auth_junction.py tests/unit/test_v150_wp2_per_row_ks.py tests/unit/test_v150_wp3_canonical_first.py tests/unit/test_v141_def012_wiring.py` → **30 passed** (exit 0).
  - **Adjacent (18-file authorization/retrieval matrix):** m5×6 + m3×4 + v160×3 + v150_wp2/wp3 + v140_02 + v141_def012/r2 → **512 passed** (exit 0).
...
- **Correction round-3 (2026-08-26, additive):** DEF-037 vẫn **REOPENED / INVESTIGATING**. Environment sensitivity đã xác nhận (khác giữa sandbox và clean env) nhưng **root cause cụ thể chưa xác nhận** — round-2 addendum đã overclaim (sandbox 0o700 / sitecustomize / "không phải defect Zero-Mem" chưa có raw executable evidence đủ). Không thể loại trừ installer/test-harness/Zero-Mem defect — **Zero-Mem defect status: NOT YET DETERMINED**. Cần supported-Python split test ngoài sandbox + inner ensurepip full stderr (nếu không lấy được → ghi INNER_STDERR_NOT_CAPTURED, không suy đoán). Full qualification BLOCKED. Technical C4/P2 VERIFIED. Không authorize C5.
