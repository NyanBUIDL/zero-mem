...
### C2 — Ingest denormalize: zm_event_spaces junction + PRIMARY-KS (migration additive) — **DONE (commit 617ed92 + follow-up)**
- Migration v13 (additive): `zm_event_spaces(event_id, knowledge_space_id, PK)` + index ks; backfill từ `zm_meta.knowledge_space_id` (legacy singular, **chỉ non-empty** — `TRIM(...) <> ''`, NULL/blank KHÔNG backfill, ADR §2; review P1); down = DROP (derived, rebuildable).
...
- **Review follow-up (commit 617ed92+):** P1 backfill lọc blank legacy (`TRIM`) — RED 1 failed → GREEN 8 passed; P2 test upgrade thật qua migration runner (`downgrade_to(12)` → insert legacy → reopen → `ensure_schema()`; assert version 13 + backfill đúng + ledger 1 row v13) + downgrade test (table+index bị loại); P3 đổi tên `test_rebuild_junction_faithful` → `test_fresh_ingest_junction_parity` (rebuild thật là C3).
- **Full-suite gate (raw evidence, review P4):** full suite sau follow-up: 3565 passed / **6 failed (đúng 6 test IDs)** / 38 skipped / 11 errors (installer quirk). 6 IDs: `test_v122_lock_modes::{shared,exclusive}`, `test_v122_platform_storage::{concurrent_process_lock,abandoned_lock}`, `test_v134_def003_crash_durability::test_sigkill_mid_ingest_preserves_canonical_and_resumes`, `test_v151_phase4_async_cancellation::test_def026_abandoned_work_is_cancelled_before_start_when_worker_frees`. **Baseline diff:** chạy chính 6 IDs này tại C1 (cfea75d, git worktree) → cả 6 đều FAIL trước C2 — pre-existing env/Windows, không regression. Raw logs: `audit/evidence-v160-c2/` (fullsuite-*, c1baseline-cfea75d-6tests-same-failures.txt, c2followup-*).
