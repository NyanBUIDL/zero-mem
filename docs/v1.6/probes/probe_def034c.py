...
# B. REAL capture: adapter -> canonical append
root = tmp / "canonical"
store = JsonlCaptureStore(CaptureStoreConfig(root))
env = normalize_event({"text": "hello", "redaction_audit": AUDIT}, profile_id="p1", project_id="P",
                      sequence=0, event_type="user_statement", source="hermes_chat")
print("envelope has knowledge_space_id:", "knowledge_space_id" in env)
...
# D. hand-crafted canonical WITH ks (validate_envelope allows unknown fields)
jl = tmp / "ks.jsonl"
_write_jsonl(jl, [
...
print("zm_meta ks (hand-crafted):", rows2)

# E. FTS SearchHit
from src.retrieval.search import search_text
ro = open_readonly(tmp / "m2.sqlite")
res = search_text(ro, "clean content")
print("FTS hits ks:", {h.event_id: h.knowledge_space_id for h in res.results})
ro.close()

# F. authorization: space grant per-row (ks rows vs NULL rows)
...
req = AccessRequest(operation="READ", requesting_profile_id="prof-owner", knowledge_space_ids=["quant-theory"])
